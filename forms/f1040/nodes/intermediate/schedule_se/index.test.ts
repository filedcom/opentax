...
function computeExpectedSeTax(
  ...
  const line6 = line4a;
  const line8d = w2SsWages + unreportedTips + wages8919;
  const line9 = Math.max(0, SS_WAGE_BASE - line8d);
...
Deno.test("threshold_wage_base_zero_w2: no W-2 wages → full SS rate applies", () => {
  const profit = 50_000;
  const result = compute({ net_profit_schedule_c: profit, w2_ss_wages: 0 });
  const s2 = findOutput(result, "schedule2");
...
Deno.test("threshold_wage_base_partial_offset: w2_ss_wages=100000 reduces SS portion", () => {
  ...
  const w2Wages = 100_000;
  const result = compute({ net_profit_schedule_c: profit, w2_ss_wages: w2Wages });
  const s2 = findOutput(result, "schedule2");
...
Deno.test("threshold_wage_base_exceeded: w2_ss_wages=200000 (> base) → still only Medicare", () => {
  const profit = 50_000;
  const result = compute({ net_profit_schedule_c: profit, w2_ss_wages: 200_000 });
  const s2 = findOutput(result, "schedule2");
...
Deno.test("edge_all_offsets: w2 + tips + 8919 fully consume wage base → only Medicare", () => {
  const profit = 80_000;
  const result = compute({
    net_profit_schedule_c: profit,
    w2_ss_wages: 100_000,
    unreported_tips_4137: 50_000,
    wages_8919: 50_000, // total offsets = 200,000 > 176,100
  });
  const s2 = findOutput(result, "schedule2");
...
Deno.test("smoke_all_fields: full scenario with C+F profit, partial W-2 offset, tips, 8919", () => {
  const cProfit = 60_000;
  const fProfit = 10_000;
  const w2Wages = 80_000;
  const tips = 5_000;
  ...
  const line8d = w2Wages + tips + wages8919;
  const line9 = Math.max(0, SS_WAGE_BASE - line8d);
  const line10 = Math.min(line6, line9) * SS_RATE;
  const line11 = line6 * MEDICARE_RATE;
