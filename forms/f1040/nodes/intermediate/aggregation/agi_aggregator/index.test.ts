...
Deno.test("agi_aggregator: wages-only produces exactly 9 outputs (no Schedule 1 items)", () => {
  // With wages only, line8=0 and line10=0 so no extra f1040 fields beyond line11_agi
  // Outputs: f1040 + standard_deduction + schedule_a + eitc + f8812 + f2441 + form8995 + form8960 + form8962
  const result = compute({ line1a_wages: 50_000 });
  assertEquals(result.outputs.length, 9);
});
