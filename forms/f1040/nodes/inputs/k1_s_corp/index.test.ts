...
function compute(items: ReturnType<typeof minimalItem>[]) {
  return k1SCorpNode.compute({ k1_s_corps: items });
}
...
Deno.test("empty array throws", () => {
  assertThrows(() => k1SCorpNode.compute({ k1_s_corps: [] }), Error);
});

Deno.test("missing corporation_name throws", () => {
  assertThrows(
    () => k1SCorpNode.compute({ k1_s_corps: [{ box1_ordinary_business: 100 } as unknown as ReturnType<typeof minimalItem>] }),
    Error,
  );
});
